        ...
        <div class="order-totals-details">
			...
			<?php
				...
				if (!empty($applied_coupons)) {
					foreach ($applied_coupons as $coupon_code) {
						...
						// Get correct discount from cart totals
						...
						$full_discount = $discount_amount + $discount_tax;
			...
			<div class="order-total order-totals-item">
				...
				<?php
					...
					// Get total as number
					$total_raw = floatval(preg_replace('/[^\d.]/', '', $cart->get_total('edit')));

					// Format as WooCommerce price
					$formatted_total = wc_price($total_raw);

					// Optional: highlight total value with span
					$formatted_total = preg_replace(
						'/(<bdi>.*?<span class="woocommerce-Price-currencySymbol">.*?<\/span>)([\d.,]+)/',
        ...
        jQuery(document).ready(function ($) {
			...
			function moveAndStyleOrderTotals() {
				if ($(window).width() <= 767) {
					...
					var elementorContainer = $(".elementor-7211");
					var orderReview = $(".e-checkout__order_review");
					...
					// Move Elementor container before Order Review
					if (elementorContainer.length && orderReview.length) {
						if (!orderReview.prev(".elementor-7211").length) {
							elementorContainer.insertBefore(orderReview);
						}
					}
				}
			}
			...
			function updateOrderTotals() {
				...
				// Get subtotal
				const subtotal = parseFloat($('.cart-subtotal .woocommerce-Price-amount bdi').text().replace(/[^\d.]/g, '')) || 0;

				// Get shipping
				const shipping = parseFloat($('.shipping-value').text().replace(/[^\d.]/g, '')) || 0;

				// Get total coupon discount (in case multiple coupons applied)
				let totalDiscount = 0;
				$('.applied-coupon .woocommerce-Price-amount bdi').each(function () {
					const discount = parseFloat($(this).text().replace(/[^\d.]/g, '')) || 0;
				...
				// Update total
				$('.order-total-value').text(finalTotal.toFixed(2));
			}
			...
			/*
			 * If "Local Pickup Free" is selected, hide the 5 shipping address fields 
			   (shipping_country, shipping_address_1, shipping_city, shipping_state, shipping_postcode).
			 * If any other shipping method is selected, show those fields again.
			 */	
			function toggleShippingAddressFields() {
				var selectedMethod = $('input[name="shipping_method[0]"]:checked').next('label').text().trim();
				// var isLocalPickup = selectedMethod.includes('Local Pickup Free');
				var isLocalPickup = $('input[name="shipping_method[0]"]:checked').val().includes('local_pickup');
				...
				var shippingFields = [
					...
					'.custom-shipping_address_heading'
				];

				if (isLocalPickup) {
					shippingFields.forEach(function(selector) {
						$(selector).stop(true, true).slideUp(300);
					});
				} else {
					shippingFields.forEach(function(selector) {
						$(selector).stop(true, true).slideDown(300);
					});
				}
			}
			...
			// Run on change of shipping method
...
function customize_checkout_fields($fields) {
	...
	// Remove "required" validation from these 5 fields
...
function render_heading_field($field, $key, $args, $value) {
	...
	if ($key === 'practitioner_information') {
        ...
        if (!$practitioner_name) {
			...
			$practitioners = get_field( 'practitioners', 'option' );
			if ($practitioners && $practitioner_name) {
				foreach ($practitioners as $practitioner) {
					if ($practitioner['name'] == $practitioner_name) {
						$practitioner_photo_url = $practitioner['avatar']['sizes']['medium'];
						break;
					}
				}
			}
...
// Automatically Use Shipping info for Billing info (This is working for like Afterpay checkout method)
add_action('woocommerce_checkout_update_order_meta', function($order_id) {
    ...
    foreach ($fields_to_copy as $field) {
        $billing_key  = '_billing_' . $field;
        $shipping_key = '_shipping_' . $field;

        $billing_value  = get_post_meta($order_id, $billing_key, true);
        $shipping_value = get_post_meta($order_id, $shipping_key, true);

        if (empty($billing_value) && !empty($shipping_value)) {
            update_post_meta($order_id, $billing_key, $shipping_value);
        }
    }
});
